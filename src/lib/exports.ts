...
export function exportProductsToExcelDetailed(
  items: StockItem[],
  currency: string,
  _company: CompanyExportProfile
) {
  const data = items.map((item) => {
    ...
    return {
      ...
      Reserve: item.reserved,
      Restant: item.remaining,
      PrixUnitaire: item.price_ht ?? "",
      Total: total,
      ClientNom: item.client?.name || "",
      ClientEmail: item.client?.email || "",
      Notes: item.notes || "",
      ImagePrincipale: item.image_url || "",
      Devise: currency,
    };
  });
  ...
  const worksheet = XLSX.utils.json_to_sheet(data);
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Produits detailles");
  ...
  const stamp = new Date().toISOString().slice(0, 10);
  saveAs(blob, `produits-detailles-${stamp}.xlsx`);
}
...
export function exportCatalogPdf(
  ...
  doc.setFontSize(16);
  doc.text(company.company_name || "Catalogue", 14, y);
  y += 8;
  doc.setFontSize(12);
  doc.text(title || "Catalogue de produits", 14, y);
  y += 10;
  doc.setFontSize(10);
  ...
  items.forEach((item, index) => {
    ...
    const total = (item.price_ht || 0) * item.quantity;
    const line = `${index + 1}. #${item.number} ${item.description} | Qté: ${item.quantity} | Prix: ${formatCurrency(item.price_ht, company.currency)} | Total: ${formatCurrency(total, company.currency)}`;
